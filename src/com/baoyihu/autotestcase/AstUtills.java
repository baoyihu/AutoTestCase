...
public class AstUtills
{
    ...
    public static CompilationUnit getRoot(String classFile)
    {
        ...
        if (classFile.contains("ParseClass.java"))
        {
            debugFlag++;
        }
    ...
    public static String getClassFullName(CompilationUnit root)
    {
        ...
        {
            TypeDeclaration thisType = types.get(0);
            PackageDeclaration declare = root.getPackage();
            if (declare != null)
            {
                ret = declare.getName().getFullyQualifiedName() + "." + thisType.getName().toString();
            }
            else
            {
                ret = thisType.getName().toString();
            }
        }
        return ret;
    }
    ...
    public static String getSimpleTypeFullName(SimpleType simpleType)
    {
	    ASTNode node = 	simpleType.getRoot();
	    if(node instanceof CompilationUnit)
	    {
	    	CompilationUnit root = (CompilationUnit)node;
	    	List<ImportDeclaration> importList = root.imports();
	    	for(ImportDeclaration declare:importList)
	    	{
	    		String name =declare.getName().getFullyQualifiedName();
	    		if(name.endsWith(simpleType.getName().getFullyQualifiedName()))
	    		{
	    			return name;
	    		}
	    	}
